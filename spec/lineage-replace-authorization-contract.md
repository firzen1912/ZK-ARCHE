...
The AZ-01..AZ-10 canonical corpus in `rust/test-vectors/replay/lineage-replace-authorization-v1.txt` covers the normalized authorization decision surface. The current AX-01..AX-15 corpus in `rust/test-vectors/replay/lineage-replace-auth-context-v2.txt` covers the concrete `iot-core` binding adapter and is consumed by Rust and C. Its negative cases independently cover invalid authorization-context shape, authorization-generation mismatch, policy-epoch mismatch, revocation-epoch mismatch, wrong peer binding, stale/wrong predecessor binding, privilege expansion, and deterministic rejection precedence. The older v1 corpus remains historical evidence; v2 separates the three authorization-lifecycle coordinates so a regression in revocation or authorization-generation binding cannot be masked by a generic attribution/context mismatch. These vectors verify equality between the locally resolved attribution record and the supplied current authorization context; they do not by themselves prove that an external store supplied the latest device-wide epoch values.
