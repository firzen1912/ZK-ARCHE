...
The resumption path MUST determine whether emitting/reusing its privacy-relevant identifier would violate the selected profile's repeated-identifier/linkability bound before emitting that identifier on the fast path. Unknown/stale privacy-identifier state is not permission to reuse an identifier. The concrete identifier construction, rotation schedule, and wire encoding remain profile-owned; this classifier consumes only the local decision facts.
...
The decision consumes local facts representing at least credential presence/integrity, binding, expiry and bounded reuse, persisted reuse-counter continuity, cached authorization context presence/freshness, authenticated authorization-generation binding and equality with the current local generation, revocation and lineage state, restart/replay continuity, credential epoch freshness, explicit invalidation, privacy-identifier state freshness, whether proposed repeated identifier use is linkable beyond the permitted profile bound, peer/deployment/audience/profile equality, and rollback suspicion.

The classifier is intentionally storage- and wire-neutral. TRUST/LINK/storage/profile specifications own the mechanisms that authenticate these facts. In particular, `privacy_identifier_state_current` means the implementation has sufficient current local state to enforce the selected profile's identifier-rotation/reuse rule; `repeated_identifier_linkable` means the proposed resumption reuse would exceed that rule or otherwise create a repeated-identifier condition the profile forbids.

## 3. Actions

`RESUME` permits only the future resumption flow to continue; it does not itself establish a new secure association.

`FULL_AUTH_REQUIRED` means the fast path MUST stop, but fresh normal AUTH may be attempted. Full AUTH remains subject to current authorization/revocation/freshness checks and MUST NOT repair trust state, invent an authorization-generation binding, synthesize current authorization, or retroactively make an already-emitted repeated identifier private.
...
Possession of a ticket/PSK MUST NOT override revocation, stale lineage, invalidation, stale restart continuity, stale reuse-counter continuity, absent/stale authorization-generation binding, stale privacy-identifier state, a forbidden repeated-identifier condition, or rollback detection.
...
Fresh AUTH MAY establish a new association only under the current authorization decision. It MUST NOT reinterpret the failed credential as generation-bound, mutate trust as a side effect, reset replay/reuse state, or reuse an identifier that the privacy policy requires to rotate.

Restart or transport reconnection MUST NOT manufacture fresh authorization, revocation, replay, reuse-counter, privacy-identifier, epoch, generation, or lineage state. Explicit session invalidation remains terminal for that session instance. Transport addresses are not protocol identity.
...
This evidence demonstrates decision parity only. It does not claim ticket issuance/protection, a concrete unlinkable identifier construction, atomic persistent invalidation, anti-rollback storage, resumption wire interoperability, formal proof, physical MCU measurements, independent review, or deployment qualification.
