...
Failure of any commissioner prerequisite MUST transition to `DENIED`. A stale commissioner authorization MUST NOT be refreshed by successful AUTH.
...
4. restart recovery cannot make both the successor usable and the consumed operation identifier reusable.
...
Recovery MUST follow `lifecycle-persistence-freshness.md`. Normal AUTH MUST NOT repair this state, clear rollback suspicion, synthesize the successor, or reset the enrollment replay domain.
...
Restart MUST NOT convert `COMMITTED` to an apparently unused enrollment operation. Transport reconnect, address change, process restart, or a new AUTH session MUST NOT reset enrollment replay state.
...
Internally, implementations SHOULD preserve the reason classes from the enrollment issuance decision corpus. Remote signaling MAY coalesce reasons to avoid creating commissioner, subject, revocation, lineage, or replay oracles.
...
ENR-SM-04 revoked commissioner -> DENIED
ENR-SM-05 failed subject possession -> DENIED
ENR-SM-06 replayed/consumed operation identifier -> DENIED
ENR-SM-07 requested authority exceeds commissioner authority -> DENIED
ENR-SM-08 stale epoch/revocation/lineage -> DENIED
ENR-SM-09 rollback suspicion -> DENIED or RECOVERY_REQUIRED; never COMMITTED
ENR-SM-10 interruption during COMMITTING -> predecessor restored or RECOVERY_REQUIRED
ENR-SM-11 restart after COMMITTED -> operation identifier remains consumed
ENR-SM-12 concurrent same-identifier attempts -> at most one COMMITTED
```
