...
fn expected(v: &str) -> LineageReplaceStorageTransactionResult { match v {
...
fn name(v: LineageReplaceStorageStep) -> &'static str { match v {
...
fn canonical_plan() -> LineageReplacePlan { LineageReplacePlan {
    retire_predecessor: true, activate_successor: true, invalidate_session_keys: true,
    invalidate_resumption: true, invalidate_authorization_cache: true,
    invalidate_attribution_cache: true, invalidate_channel_binding: true,
    invalidate_replay_state: true,
} }

#[test]
fn canonical_lineage_replace_storage_transaction_corpus() {
    let corpus = include_str!("../../../test-vectors/replay/lineage-replace-storage-transaction-v1.txt");
    let mut cases = 0usize;
    for line in corpus.lines() {
        if !line.starts_with("case=") { continue; }
        let f: Vec<&str> = line[5..].split('|').collect(); assert_eq!(f.len(), 5);
        let mut plan = canonical_plan(); if !bit(f[1]) { plan.invalidate_replay_state = false; }
        let fail = step(f[2]); let mut trace: Vec<&'static str> = Vec::new();
        let result = execute_lineage_replace_storage_transaction(Some(&plan), |s| {
            trace.push(name(s)); Some(s) != fail
        });
        assert_eq!(result, expected(f[3]));
        let actual = if trace.is_empty() { "NONE".to_string() } else { trace.join(",") };
        assert_eq!(actual, f[4]); cases += 1;
    }
    assert_eq!(cases, 7);
}
