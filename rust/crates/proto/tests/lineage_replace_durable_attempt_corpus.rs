...
fn attempt(decision: &str) -> LineageReplaceAttemptDecision {
    let mut f = LineageReplaceAttemptFacts {
...
fn durable_attempt_corpus() {
    let corpus =
        include_str!("../../../test-vectors/replay/lineage-replace-durable-attempt-v1.txt");
    ...
    for line in corpus.lines().filter(|l| l.starts_with("case=")) {
        ...
        let recovered = recover_lineage_replace_with_freshness(
            &recovery(p[4]),
        ...
        let operable = match p[5] {
            "RESUME_SUCCESSOR" => {
                decision == LineageReplaceAttemptDecision::Converged
                    && recovered == LineageReplaceState::ActiveSuccessorPredecessorRetired
            }
            "RESUME_PREDECESSOR" => recovered == LineageReplaceState::ActivePredecessor,
            "CONTINUITY_BROKEN" => {
                decision != LineageReplaceAttemptDecision::Converged
                    || recovered == LineageReplaceState::ContinuityBroken
            }
