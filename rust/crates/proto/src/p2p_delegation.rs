...
//! trust state and every explicit scope, validity, epoch, revocation, lineage,
//! depth, and redelegation guard is satisfied. Trust inferred only through a
//! third party is not an acceptable issuer trust root. This module consumes
//! already-verified local facts; it does not parse or cryptographically verify
//! a grant.
...
mod tests {
    ...
    fn action(value: &str) -> P2pDelegationAction {
        match value {
            "ACCEPT" => P2pDelegationAction::Accept,
            "DENY" => P2pDelegationAction::Deny,
    ...
    fn canonical_v2_corpus_matches_classifier() {
        let corpus = include_str!("../../../test-vectors/p2p/bounded-delegation-v2.txt");
        ...
        for line in corpus.lines() {
            ...
            let facts = P2pDelegationFacts {
                issuer_trusted: bit(fields[1]),
                issuer_trust_local: bit(fields[2]),
                holder_authenticated: bit(fields[3]),
                grant_present: bit(fields[4]),
                grant_integrity_valid: bit(fields[5]),
                scope_match: bit(fields[6]),
                audience_match: bit(fields[7]),
                deployment_match: bit(fields[8]),
                validity_current: bit(fields[9]),
                epoch_current: bit(fields[10]),
                revocation_current: bit(fields[11]),
                explicitly_revoked: bit(fields[12]),
                lineage_current: bit(fields[13]),
                depth_within_limit: bit(fields[14]),
                redelegation_permitted: bit(fields[15]),
                redelegation_requested: bit(fields[16]),
                rollback_suspected: bit(fields[17]),
            };
            assert_eq!(
                classify_p2p_delegation(&facts),
                P2pDelegationDecision {
                    action: action(fields[18]),
                    reason: reason(fields[19]),
                },
                "case {}",
                fields[0]
            );
