...
pub fn classify_lineage_replace_authorization(
    facts: &LineageReplaceAuthorizationFacts,
) -> LineageReplaceAuthorizationDecision {
    use LineageReplaceAuthorizationDecision::*;
    if !facts.current_credential_control_valid {
        return RejectCurrentCredentialControl;
    }
    if !facts.successor_key_control_valid {
        return RejectSuccessorKeyControl;
    }
    if !facts.current_session_authenticated {
        return RejectSessionAuthentication;
    }
    if !facts.current_session_authorized {
        return RejectSessionAuthorization;
    }
    if !facts.current_context_bound {
        return RejectContextBinding;
    }
    if !facts.predecessor_binding_current {
        return RejectPredecessorBinding;
    }
    if !facts.successor_scope_within_authorized_scope {
        return RejectPrivilegeExpansion;
    }
    AuthorizedReplacement
}
...
pub fn evaluate_authorized_lineage_replace(
    authorization: LineageReplaceAuthorizationDecision,
    facts: &LineageReplaceFacts,
) -> LineageReplaceDecision {
    let mut normalized = *facts;
    normalized.authority_valid =
        authorization == LineageReplaceAuthorizationDecision::AuthorizedReplacement;
