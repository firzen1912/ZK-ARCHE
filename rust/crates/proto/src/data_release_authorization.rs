...
pub fn classify_data_release(f: &DataReleaseFacts) -> DataReleaseDecision {
    ...
    if !f.authorization_generation_bound {
        return decision(
            DataReleaseAction::Deny,
            DataReleaseReason::AuthorizationGenerationUnbound,
        );
    }
    ...
    fn canonical_v4_corpus_matches_classifier() {
        ...
        for line in corpus.lines() {
            ...
            let facts = DataReleaseFacts {
                ...
                authorization_generation_bound: bit(fields[8]),
                authorization_generation_current: bit(fields[9]),
                revocation_current: bit(fields[10]),
                explicitly_revoked: bit(fields[11]),
                lineage_current: bit(fields[12]),
                holder_match: bit(fields[13]),
                audience_match: bit(fields[14]),
                purpose_match: bit(fields[15]),
                data_type_match: bit(fields[16]),
                policy_match: bit(fields[17]),
                epoch_match: bit(fields[18]),
                channel_binding_required: bit(fields[19]),
                channel_binding_valid: bit(fields[20]),
                release_operation_unused: bit(fields[21]),
                rollback_suspected: bit(fields[22]),
            };
            assert_eq!(
                classify_data_release(&facts),
                DataReleaseDecision {
                    action: action(fields[23]),
                    reason: reason(fields[24])
                },
                "case {}",
                fields[0]
            );
