...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DataReleaseReason { Current, RollbackSuspected, Unauthenticated, DeviceReleaseAuthorityMissing, DeviceReleaseAuthorityStale, ProtectedDataNotEncrypted, ReleaseKeyScopeMismatch, AuthorizationMissing, AuthorizationStale, AuthorizationGenerationUnbound, AuthorizationGenerationStale, RevocationStale, Revoked, LineageStale, HolderMismatch, AudienceMismatch, PurposeMismatch, DataTypeMismatch, PolicyMismatch, EpochMismatch, ChannelBindingMissingOrInvalid, ReleaseReplayDetected }
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct DataReleaseDecision { pub action: DataReleaseAction, pub reason: DataReleaseReason }
fn decision(action: DataReleaseAction, reason: DataReleaseReason) -> DataReleaseDecision { DataReleaseDecision { action, reason } }
...
mod tests {
    ...
    fn reason(value: &str) -> DataReleaseReason { match value {
        "CURRENT" => DataReleaseReason::Current, "ROLLBACK_SUSPECTED" => DataReleaseReason::RollbackSuspected, "UNAUTHENTICATED" => DataReleaseReason::Unauthenticated,
        "DEVICE_RELEASE_AUTHORITY_MISSING" => DataReleaseReason::DeviceReleaseAuthorityMissing, "DEVICE_RELEASE_AUTHORITY_STALE" => DataReleaseReason::DeviceReleaseAuthorityStale,
        "PROTECTED_DATA_NOT_ENCRYPTED" => DataReleaseReason::ProtectedDataNotEncrypted, "RELEASE_KEY_SCOPE_MISMATCH" => DataReleaseReason::ReleaseKeyScopeMismatch,
        "AUTHORIZATION_MISSING" => DataReleaseReason::AuthorizationMissing, "AUTHORIZATION_STALE" => DataReleaseReason::AuthorizationStale,
        "AUTHORIZATION_GENERATION_UNBOUND" => DataReleaseReason::AuthorizationGenerationUnbound, "AUTHORIZATION_GENERATION_STALE" => DataReleaseReason::AuthorizationGenerationStale,
        "REVOCATION_STALE" => DataReleaseReason::RevocationStale, "REVOKED" => DataReleaseReason::Revoked, "LINEAGE_STALE" => DataReleaseReason::LineageStale,
        "HOLDER_MISMATCH" => DataReleaseReason::HolderMismatch, "AUDIENCE_MISMATCH" => DataReleaseReason::AudienceMismatch, "PURPOSE_MISMATCH" => DataReleaseReason::PurposeMismatch,
        "DATA_TYPE_MISMATCH" => DataReleaseReason::DataTypeMismatch, "POLICY_MISMATCH" => DataReleaseReason::PolicyMismatch, "EPOCH_MISMATCH" => DataReleaseReason::EpochMismatch,
        "CHANNEL_BINDING_MISSING_OR_INVALID" => DataReleaseReason::ChannelBindingMissingOrInvalid, "RELEASE_REPLAY_DETECTED" => DataReleaseReason::ReleaseReplayDetected,
    ...
    fn canonical_v4_corpus_matches_classifier() {
        ...
        for line in corpus.lines() {
            ...
            let facts = DataReleaseFacts { authenticated: bit(fields[1]), device_release_authority_present: bit(fields[2]), device_release_authority_current: bit(fields[3]), protected_data_encrypted: bit(fields[4]), release_key_scope_match: bit(fields[5]), authorization_present: bit(fields[6]), authorization_fresh: bit(fields[7]), authorization_generation_bound: bit(fields[8]), authorization_generation_current: bit(fields[9]), revocation_current: bit(fields[10]), explicitly_revoked: bit(fields[11]), lineage_current: bit(fields[12]), holder_match: bit(fields[13]), audience_match: bit(fields[14]), purpose_match: bit(fields[15]), data_type_match: bit(fields[16]), policy_match: bit(fields[17]), epoch_match: bit(fields[18]), channel_binding_required: bit(fields[19]), channel_binding_valid: bit(fields[20]), release_operation_unused: bit(fields[21]), rollback_suspected: bit(fields[22]) };
