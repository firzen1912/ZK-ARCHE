...
mod tests {
    ...
    fn canonical_v3_corpus_matches_classifier() {
        let corpus = include_str!("../../../test-vectors/state/data-release-authorization-v3.txt");
        ...
        for line in corpus.lines() {
            ...
            let facts = DataReleaseFacts {
                ...
                authorization_generation_current: bit(fields[8]),
                revocation_current: bit(fields[9]),
                explicitly_revoked: bit(fields[10]),
                lineage_current: bit(fields[11]),
                holder_match: bit(fields[12]),
                audience_match: bit(fields[13]),
                purpose_match: bit(fields[14]),
                data_type_match: bit(fields[15]),
                policy_match: bit(fields[16]),
                epoch_match: bit(fields[17]),
                channel_binding_required: bit(fields[18]),
                channel_binding_valid: bit(fields[19]),
                release_operation_unused: bit(fields[20]),
                rollback_suspected: bit(fields[21]),
            };
            assert_eq!(
                classify_data_release(&facts),
                DataReleaseDecision {
                    action: action(fields[22]),
                    reason: reason(fields[23])
                },
                "case {}",
                fields[0]
            );
