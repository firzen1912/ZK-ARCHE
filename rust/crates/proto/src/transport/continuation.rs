...
pub fn classify_transport_continuation(
    f: &TransportContinuationFacts,
) -> TransportContinuationDecision {
    use TransportContinuationAction::*;
    use TransportContinuationReason::*;
    if f.transport_address_as_identity {
        return decision(Reject, TransportAddressAsIdentity);
    }
    if f.transport_metadata_as_authority {
        return decision(Reject, TransportMetadataAsAuthority);
    }
    if f.association_invalidated {
        return decision(Reject, AssociationInvalidated);
    }
    if !f.replay_continuity_current {
        return decision(Reject, ReplayContinuityStale);
    }
    if !f.usage_counter_continuity_current {
        return decision(Reject, UsageCounterContinuityStale);
    }
    if !f.authorization_generation_bound {
        return decision(FullAuthRequired, AuthorizationGenerationUnbound);
    }
    if !f.authorization_generation_current {
        return decision(FullAuthRequired, AuthorizationGenerationStale);
    }
    if !f.authenticated_peer_context_match {
        return decision(Reject, PeerContextMismatch);
    }
    if !f.authenticated_profile_context_match {
        return decision(FullAuthRequired, ProfileContextMismatch);
    }
    if f.binding_required && !f.binding_valid {
        return decision(Reject, BindingInvalid);
    }
    if !f.resumption_authorized {
        return decision(FullAuthRequired, ResumptionNotAuthorized);
    }
    if !f.continuation_requested {
        return decision(FullAuthRequired, ContinuationNotRequested);
    }
...
mod tests {
    ...
    fn action(v: &str) -> TransportContinuationAction {
        match v {
            ...
            _ => panic!("invalid action: {v}"),
        }
    }
    fn reason(v: &str) -> TransportContinuationReason {
        match v {
            ...
            "USAGE_COUNTER_CONTINUITY_STALE" => {
                TransportContinuationReason::UsageCounterContinuityStale
            }
            "AUTHORIZATION_GENERATION_UNBOUND" => {
                TransportContinuationReason::AuthorizationGenerationUnbound
            }
            "AUTHORIZATION_GENERATION_STALE" => {
                TransportContinuationReason::AuthorizationGenerationStale
            }
    ...
    fn canonical_corpus_matches_classifier() {
        ...
        for line in corpus.lines() {
            ...
            let facts = TransportContinuationFacts {
                ...
                usage_counter_continuity_current: bit(fields[8]),
                authorization_generation_bound: bit(fields[9]),
                authorization_generation_current: bit(fields[10]),
                association_invalidated: bit(fields[11]),
                transport_route_changed: bit(fields[12]),
                transport_connection_changed: bit(fields[13]),
                transport_address_as_identity: bit(fields[14]),
