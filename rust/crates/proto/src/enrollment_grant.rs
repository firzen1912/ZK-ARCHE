    ...
    fn canonical_enrollment_grant_corpus() {
        ...
        for line in corpus.lines() {
            ...
            let facts = EnrollmentGrantFacts {
                explicit_enroll_operation: b(f[1]), normal_auth_path: b(f[2]), commissioner_authenticated: b(f[3]), commissioner_authorized: b(f[4]), commissioner_not_revoked: b(f[5]), subject_possession_verified: b(f[6]), requested_authority_within_commissioner_scope: b(f[7]), scope_bounded: b(f[8]), audience_bound: b(f[9]), deployment_bound: b(f[10]), validity_bounded: b(f[11]), epoch_current: b(f[12]), revocation_current: b(f[13]), lineage_current: b(f[14]), delegation_depth_within_limit: b(f[15]), rollback_suspected: b(f[16]),
            };
            let d = classify_enrollment_grant(&facts);
            let action = match d.action { EnrollmentGrantAction::Issue => "ISSUE", EnrollmentGrantAction::Deny => "DENY" };
            let normalized = match d.reason {
                EnrollmentGrantReason::Current => "CURRENT", EnrollmentGrantReason::RollbackSuspected => "ROLLBACK_SUSPECTED", EnrollmentGrantReason::NormalAuthForbidden => "NORMAL_AUTH_FORBIDDEN", EnrollmentGrantReason::ExplicitEnrollRequired => "EXPLICIT_ENROLL_REQUIRED", EnrollmentGrantReason::CommissionerUnauthenticated => "COMMISSIONER_UNAUTHENTICATED", EnrollmentGrantReason::CommissionerUnauthorized => "COMMISSIONER_UNAUTHORIZED", EnrollmentGrantReason::CommissionerRevoked => "COMMISSIONER_REVOKED", EnrollmentGrantReason::SubjectPossessionMissing => "SUBJECT_POSSESSION_MISSING", EnrollmentGrantReason::AuthorityEscalation => "AUTHORITY_ESCALATION", EnrollmentGrantReason::ScopeUnbounded => "SCOPE_UNBOUNDED", EnrollmentGrantReason::AudienceUnbound => "AUDIENCE_UNBOUND", EnrollmentGrantReason::DeploymentUnbound => "DEPLOYMENT_UNBOUND", EnrollmentGrantReason::ValidityUnbounded => "VALIDITY_UNBOUNDED", EnrollmentGrantReason::EpochStale => "EPOCH_STALE", EnrollmentGrantReason::RevocationStale => "REVOCATION_STALE", EnrollmentGrantReason::LineageStale => "LINEAGE_STALE", EnrollmentGrantReason::DelegationDepthExceeded => "DELEGATION_DEPTH_EXCEEDED",
            };
