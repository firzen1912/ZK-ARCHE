...
use crate::crypto::{
    compute_pid, derive_device_id, derive_device_scalar, derive_kc_keys,
    derive_session_key, hmac_tag, kc_transcript_hash, prove_auth_client,
    prove_auth_server, prove_rerandomization, prove_role_set_membership,
    random_bytes_32, random_scalar, reject_identity, rerandomize_commitment,
    verify_auth_client, verify_rerandomization, verify_role_set_membership,
};
use crate::error::{ErrorCode, ProtoError, Result};
use crate::profile::Profile;
use crate::store::{
    fs::replay_key, CredentialStore, RegistryStore, ReplayCache, ServerKeyStore,
};
...
pub fn run_auth_client<T: ClientTransport, C: CredentialStore>(
    ...
    let role = store.load_role_credential()?.ok_or_else(|| {
        ProtoError::wire(ErrorCode::CredentialMissing, "role credential missing")
    })?;
    ...
    let rerand_proof = prove_rerandomization(&role.commitment, &c_prime, &delta, &pid, &nonce_c, &eph_c);
    let branches = prove_role_set_membership(
        allowed_roles, &c_prime, role.role_code, &blind_prime, &pid, &nonce_c, &eph_c,
    );
    ...
    let a2_bytes = send_expect(transport, profile, PKT_AUTH_1, &session_id, 0, &a1.encode(), PKT_AUTH_2)?;
    let a2 = Auth2::decode(&a2_bytes)?;

    // Pin check (redundant vs transcript but catches obvious MITM early).
    if !bool::from(a2.server_pub.compress().to_bytes().ct_eq(server_pub.compress().as_bytes())) {
        x.zeroize(); eph_secret.zeroize();
        return Err(ProtoError::wire(ErrorCode::PeerKeyMismatch, "server pubkey mismatch"));
    }

    if !crate::crypto::verify_auth_server(&a2.server_pub, &a2.nonce_s, &a2.eph_s, &a2.server_proof) {
        x.zeroize(); eph_secret.zeroize();
        return Err(ProtoError::wire(ErrorCode::ProofVerifyFailed, "server auth proof invalid"));
    }

    let session_key = derive_session_key(
        &eph_secret, &a2.eph_s,
        &nonce_c, &a2.nonce_s, &pid, &eph_c, &a2.eph_s,
    );
    let th = kc_transcript_hash(
        &pid,
        &client_proof.a, &client_proof.s,
        &nonce_c, &eph_c,
        &a2.server_pub,
        &a2.server_proof.a, &a2.server_proof.s,
        &a2.nonce_s, &a2.eph_s,
    );
    ...
    if !bool::from(expected_tag_s.ct_eq(&a2.tag_s)) {
        x.zeroize(); eph_secret.zeroize();
        return Err(ProtoError::wire(ErrorCode::KeyConfirmFailed, "server finished tag mismatch"));
    }

    // --- AUTH_3 ---
    let tag_c = hmac_tag(&k_c2s, b"client finished", &th);
    let a3 = Auth3 { tag_c };
    let ack_bytes = send_expect(transport, profile, PKT_AUTH_3, &session_id, 1, &a3.encode(), PKT_AUTH_ACK)?;
...
pub fn handle_auth_1<K: ServerKeyStore, R: RegistryStore, P: ReplayCache>(
    ...
    if !verify_auth_client(&rec.pubkey, &a1.pid, &a1.nonce_c, &a1.eph_c, &a1.client_proof) {
        return Err(ProtoError::wire(ErrorCode::ProofVerifyFailed, "client auth proof invalid"));
    }

    // 4. Verify re-randomization proof.
    if !verify_rerandomization(
        &rec.role_commitment, &a1.c_prime, &a1.pid, &a1.nonce_c, &a1.eph_c, &a1.rerand_proof,
    ) {
        return Err(ProtoError::wire(ErrorCode::ProofVerifyFailed, "rerand proof invalid"));
    }

    // 5. Verify role set-membership proof.
    if !verify_role_set_membership(
        allowed_roles, &a1.c_prime, &a1.pid, &a1.nonce_c, &a1.eph_c, &a1.branches,
    ) {
        return Err(ProtoError::wire(ErrorCode::RoleNotPermitted, "role set proof invalid"));
    }
    ...
    let th = kc_transcript_hash(
        &a1.pid,
        &a1.client_proof.a, &a1.client_proof.s,
        &a1.nonce_c, &a1.eph_c,
        &server_pub,
        &server_proof.a, &server_proof.s,
        &nonce_s, &eph_s,
    );
    let (k_s2c, k_c2s) = derive_kc_keys(&session_key, &th);
...
pub fn handle_auth_3(
    ...
    if !bool::from(a3.tag_c.ct_eq(&pending.expected_tag_c)) {
        return Err(ProtoError::wire(ErrorCode::KeyConfirmFailed, "client finished tag mismatch"));
    }
