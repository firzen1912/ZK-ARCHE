...
pub fn classify_resumption_authorization(
    ...
    if !f.authorization_generation_bound {
        return d(FullAuthRequired, AuthorizationGenerationUnbound);
    }
...
mod tests {
    ...
    fn r(v: &str) -> ResumptionReason {
        ...
        match v {
            ...
            "AUTHORIZATION_GENERATION_UNBOUND" => AuthorizationGenerationUnbound,
    ...
    fn canonical_corpus() {
        ...
        for line in corpus.lines().filter(|l| l.starts_with("case=")) {
            ...
            let f = ResumptionAuthorizationFacts {
                ...
                authorization_generation_bound: b(x[10]),
                authorization_generation_current: b(x[11]),
                revocation_current: b(x[12]),
                explicitly_revoked: b(x[13]),
                lineage_current: b(x[14]),
