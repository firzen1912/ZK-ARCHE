...
|---|---|---|---|---|---|
| R-001 | EDHOC/OSCORE-inspired constrained binding, exporter, and explicit profile-negotiation semantics | benchmark | exact ZK-ARCHE-vs-EDHOC wire/footprint comparison; explicit field-ownership matrix separating immutable profile contract, non-authoritative support advertisement, and per-session selected/authenticated context; prescriptive-vs-non-prescriptive parameter classification; stable profile-ID immutability and same-ID Rust/C/cross-build fingerprint parity; explicit absence/default semantics; ordered-preference vs unordered-support-set semantics; advertisement/retry-hint non-authority with forged/stale-hint tests; exporter-parameter ownership (fixed profile value vs per-session authenticated selection); transcript binding of final selected version/suite/profile/capabilities; downgrade/unknown/incompatible-profile behavior; versioned replacement/deprecation behavior; interop implications | `docs/roadmaps/rfc-evolution-plan.md` / `spec/` | 2026-09-26 |
...
| R-007 | CFRG-informed conformance/security review of the CDS OR-composed role-membership proof | reproduce | mathematical proof contract; simulator/completeness/soundness/ZK assumption matrix; canonical unique role-list semantics including ordering, duplicate rejection, empty/max-cardinality behavior; Fiat–Shamir session/instance separation covering protocol version/method/suite/profile or a compositional proof of equivalent higher-layer binding; witness-independent real/simulated clause control flow, memory access and randomness schedule or an explicitly scoped/measured side-channel claim; exact compiler/toolchain/optimization/target/crypto-backend provenance with retained artifact identity; post-compilation control-flow/generated-code review plus target timing/power leakage tests across hidden-role/witness classes; nonzero-scalar sampling review; Rust/C canonical point/scalar parse parity; malformed/identity/noncanonical differential and negative vectors; independent cryptographic and side-channel review | TD-001 / assurance / future proof ADR/spec text | 2026-09-25 |
