...
r = run(fake)
if r.returncode == 0:
    raise SystemExit("constrained target validator self-test: fabricated unmeasured observation was accepted")
...
r = run(fake)
if r.returncode == 0:
    raise SystemExit("constrained target validator self-test: context-free measured claim was accepted")

print("constrained-target-manifest-self-test: PASS negative_cases=2")
