...
# produced it, and retained evidence. Without such a record, "exact-current dev
...
gate_path="$ROOT/scripts/ci-all.sh"
if [ ! -f "$gate_path" ]; then
  echo "qualification record: NOT RECORDED (qualification gate missing: scripts/ci-all.sh)" >&2
  exit 1
fi
...
if [ -n "$LOG_PATH" ]; then
  if [ ! -f "$LOG_PATH" ]; then
    echo "qualification record: NOT RECORDED (requested CI log does not exist: $LOG_PATH)" >&2
    exit 1
  fi
...
echo "qualification record: PASS ($REPO_SHORT; evidence=${MANIFEST#"$ROOT/"}; gate_sha256=$gate_sha256; log_sha256=$retained_log_sha256)"
