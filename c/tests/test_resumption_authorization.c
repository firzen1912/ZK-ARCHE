...
#define VECTOR_PATH "../rust/test-vectors/state/resumption-authorization-v1.txt"
...
static resumption_reason_t reason(const char *value) {
    static const char *const names[] = {
        ...
        "REVOKED", "LINEAGE_STALE", "PEER_MISMATCH", "DEPLOYMENT_MISMATCH",
        "AUDIENCE_MISMATCH", "PROFILE_MISMATCH"};
    size_t i;
...
int main(void) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        for (i = 1u; i < 19u; ++i) fields[i] = strtok(NULL, "|");
        assert(fields[18] != NULL && strtok(NULL, "|") == NULL);
        ...
        assert(got.action == action(fields[17]));
        assert(got.reason == reason(fields[18]));
        cases += 1u;
    }
    fclose(fp);
    ...
    puts("resumption authorization corpus: ok");
    return EXIT_SUCCESS;
}
