...
#define VECTOR_PATH "../rust/test-vectors/state/association-admission-v3.txt"
...
int main(void) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        char *fields[18]; size_t i; association_admission_facts_t facts; association_admission_decision_t got;
        line[strcspn(line, "\r\n")] = '\0'; if (strcmp(line, "version=3") == 0) { saw_version = 1; continue; } if (strncmp(line, "case=", 5u) != 0) continue;
        fields[0] = strtok(line + 5u, "|"); for (i = 1u; i < 18u; ++i) fields[i] = strtok(NULL, "|"); assert(fields[17] != NULL && strtok(NULL, "|") == NULL);
        facts = (association_admission_facts_t){bit(fields[1]),bit(fields[2]),bit(fields[3]),bit(fields[4]),bit(fields[5]),bit(fields[6]),bit(fields[7]),bit(fields[8]),bit(fields[9]),bit(fields[10]),bit(fields[11]),bit(fields[12]),bit(fields[13]),bit(fields[14]),bit(fields[15])};
        got = association_admission_classify(&facts); assert(got.action == action(fields[16])); assert(got.reason == reason(fields[17])); cases += 1u;
    }
    fclose(fp); assert(saw_version == 1); assert(cases == 17u);
    ...
    puts("association admission corpus v3: ok cases=17"); return EXIT_SUCCESS;
}
