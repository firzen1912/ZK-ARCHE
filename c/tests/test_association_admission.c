...
static association_admission_action_t action(const char *value) {
    if (strcmp(value, "ESTABLISH") == 0) return ASSOCIATION_ADMISSION_ESTABLISH;
    assert(strcmp(value, "FAIL_CLOSED") == 0);
    return ASSOCIATION_ADMISSION_FAIL_CLOSED;
}
...
static association_admission_reason_t reason(const char *value) {
    ...
    return ASSOCIATION_ADMISSION_REASON_INVALID_FACTS;
}
